...
The application is organized into **bounded contexts**, most of which are enforced at compile time by the `boundary` library (`use Boundary` in the context's root module, owning its schemas and exposing a narrow public surface via `exports:`). Fourteen contexts are bounded. Four fall **outside** that enforcement: `Slackex.Sous`, `Slackex.Factory`, `Slackex.Analytics`, and `Slackex.Markdown` root modules neither `use Boundary` nor appear in the `mix.exs` boundary `:ignore` list, so `mix boundary.spec` does not see them at all. This is an **unenforced gap, not a documented exclusion** — every one of these contexts was created after the bounding convention was established (the last boundary was added to `Integrations` on 2026-03-19; these post-date it) and none was retrofitted. The gap is silent because Boundary is not configured to warn on unclassified modules, so their dependencies are unconstrained and they expose no checked `exports`. The contexts are:
