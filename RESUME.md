...
- **B1 BUILT (2026-05-28)** — `Slackex.Sous.Viewer` (data-driven role-lenses, 7 seeded) + `Slackex.Sous.WorkItemFacet` (composite PK, lazy default `:watch`) + `:attention_set` event + `Sous.set_attention/4` command + `Slackex.Sous.ViewerPreference` encapsulated seam (LocalStorage default, InMemoryStore proves the swap). In Service board reshapes per the active viewer's attention (`act > watch > know`, hidden via `+N not at your altitude` toggle). Facet Drawer renders the same atom through each prism's attention pill. **Behind `:sous` (off in prod). Not yet pushed/deployed.** New invariants #8–#11 in the spec (`docs/feature/sous/design/slice-b1-role-lens-and-facet-drawer.md`); plan committed at `docs/superpowers/plans/2026-05-28-sous-slice-b1-role-lens.md`. AI per-role facet text is deferred to **B2**.
